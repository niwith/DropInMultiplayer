...
namespace DropInMultiplayer
{
    internal static class ItemsHelper
    {
        ...
        // Ignore void items until I figure out how to deal with them, probably will need to untangle a lot of this code
        public static ItemTier[] ItemTiers { get; } = ((ItemTier[])Enum.GetValues(typeof(ItemTier)))
            .Except(new[] { ItemTier.NoTier, ItemTier.VoidTier1, ItemTier.VoidTier2, ItemTier.VoidTier3, ItemTier.VoidBoss }).ToArray();
        ...
        {
            CharacterMaster characterMaster = joiningPlayer.master;
            NetworkUser[] otherPlayers = NetworkUser.readOnlyInstancesList
            ...
            NetworkUser copyFrom = otherPlayers[UnityEngine.Random.Range(0, otherPlayers.Length)];
        ...
        {
            IEnumerable<ItemIndex> validDropItems = GetValidDropItems(itemTier);

            // Average of other player inventory count for this tier, minus target players current count in the tier
            int difference = (int)otherPlayerInventories.Average(inv => GetItemCountWithExclusions(inv, itemTier)) - GetItemCountWithExclusions(targetInventory, itemTier);
        ...
        {
            IEnumerable<ItemIndex> validCountItems = GetValidCountItems(itemTier);
